...
def step3_reply_matching(step2_df):
    ...
    for start_idx, start_row in start_messages.iterrows():
        ...
        if best_reply:
            ...
            # Find reply index for debug
            reply_idx = None
            for ridx, rrow in step3_df.iterrows():
                if (rrow['trader_name'] == best_reply['trader'] and 
                    rrow['bank_name'] == best_reply['bank'] and
                    ridx > start_idx):
                    reply_idx = ridx
                    break
            
            step3_df.at[start_idx, 'debug_start_reply'] = f"{debug_info} → REPLY[{reply_idx}]: {best_reply['trader']} ({best_reply['bank']}) vol={best_reply['volume']} gap={best_reply['time_gap']:.1f}min"
        else:
            problems = ["no_reply_found"]
            step3_df.at[start_idx, 'reply_problems'] = ';'.join(problems)
            step3_df.at[start_idx, 'debug_start_reply'] = f"{debug_info} → NO REPLY"
...
def step4_confirmation_processing(step3_df):
    ...
    for start_idx, start_row in start_with_reply.iterrows():
        ...
        # DEBUG: Log START-CONFIRM matching
        start_debug = step4_df.at[start_idx, 'debug_start_reply']
        
        # Update both START and CONFIRM messages
        if best_confirm:
            # Update START with link info
            step4_df.at[start_idx, 'confirm_found'] = True
            step4_df.at[start_idx, 'linked_confirm_idx'] = confirm_idx
            
            # Update CONFIRM with full sequence info
            step4_df.at[confirm_idx, 'confirm_status'] = best_confirm['status']
            step4_df.at[confirm_idx, 'confirm_volume'] = best_confirm['volume']
            step4_df.at[confirm_idx, 'confirm_trader'] = best_confirm['trader']
            step4_df.at[confirm_idx, 'linked_start_idx'] = start_idx
            step4_df.at[confirm_idx, 'debug_full_sequence'] = f"{start_debug} → CONFIRM[{confirm_idx}]: {best_confirm['trader']} status={best_confirm['status']}"
            
            # Copy START data to CONFIRM row for final processing
...
def step5_deal_assembly(step4_df):
    ...
    # Process CONFIRM messages (not START messages)
    ...
    for idx, row in confirmed_deals.iterrows():
        ...
        # Resolve volume
        volumes = [v for v in [row['start_entity_volume'], row['reply_volume'], row['confirm_volume']] if pd.notna(v)]
        if volumes:
        ...
        # Mark as valid deal
        # DEBUG: Log final deal assembly on CONFIRM row
        full_sequence = step5_df.at[idx, 'debug_full_sequence'] if 'debug_full_sequence' in step5_df.columns else "No sequence"
        
        if not problems:
            # Mark CONFIRM row as valid deal
            ...
            step5_df.at[idx, 'entity_price'] = chat_price  # For final output
...
def main():
    ...
    intermediate_file = r'D:\Projects\03.FI_crawldata\03_Task_Extract_ChatRoom\debug_detailed_results.csv'
    ...
    if len(final_df) > 0:
        # Save final results
        output_file = r'D:\Projects\03.FI_crawldata\03_Task_Extract_ChatRoom\solution_debug_results.csv'
