...
def step3_reply_matching(step2_df):
    ...
    for start_idx, start_row in start_messages.iterrows():
        ...
        # WINDOW LOGIC: Tìm START tiếp theo của cùng trader để xác định window
        ...
        window_end_idx = len(step3_df)  # Default: end of day
        if not next_start_same_trader.empty:
            window_end_idx = next_start_same_trader.index[0]
        
        # Find replies trong window (giữa 2 START của cùng trader)
        potential_replies = step3_df[
            (step3_df['intent_type'] == 'REPLY') &
            (step3_df['date'] == start_date) &
            (step3_df.index > start_idx) &
            (step3_df.index < window_end_idx) &  # TRONG WINDOW
            (step3_df['trader_name'] != start_trader)
        ]
...
def step4_confirmation_processing(step3_df):
    ...
    for start_idx, start_row in start_with_reply.iterrows():
        ...
        # WINDOW LOGIC: Tìm START tiếp theo của cùng trader cho CONFIRM
        ...
        confirm_window_end_idx = len(step4_df)  # Default: end of day
        if not next_start_same_trader.empty:
            confirm_window_end_idx = next_start_same_trader.index[0]
        
        # Find confirmations trong window (giữa 2 START của cùng trader)
        potential_confirms = step4_df[
            (step4_df['intent_type'] == 'CONFIRM') &
            (step4_df['date'] == start_date) &
            (step4_df.index > start_idx) &
            (step4_df.index < confirm_window_end_idx) &  # TRONG WINDOW
...
def main():
    ...
    intermediate_file = r'D:\Projects\03.FI_crawldata\03_Task_Extract_ChatRoom\intermediate_v2_results.csv'
    ...
    if len(final_df) > 0:
        # Save final results
        output_file = r'D:\Projects\03.FI_crawldata\03_Task_Extract_ChatRoom\solution_v2_results.csv'
